...
```math 
\mathit{"\mathbf{Veoma}\ (\rightarrow MOD=1.2)\ \mathbf{dobar}\ (p=+0.43)\ film..." \xrightarrow[]{\quad1.2 \times (+0.43)\quad } +0.52}
```
...
```math
\mathit{"Film\ \mathbf{nije}\ (\rightarrow$ NEG)\ \mathbf{zanimljiv}" (p=+0.53) \xrightarrow[\quad\times (-1)\quad]{+0.53 } -0.53}
```
...
```math 
\mathit{"Odličan film\mathbf{!}"(\leftarrow\times 1.06)} \xrightarrow[\quad\times 1.06\quad]{p=+0.57}+0.60
```
...
```math 
\mathit{"Tako\ \mathbf{dooooosadan} (p=-0.24)..." \xrightarrow[1.27]{\quad\times 1.05^{chr(o)\quad} } "Tako\ \mathbf{dosadan}..."(p=-0.30)}
```
