...
```math 
\mathit{"\mathbf{Veoma}\ (\rightarrow MOD=1.2)\ \mathbf{dobar}\ (p=+0.43)\ film..." \xrightarrow[]{\hspace{1cm}1.2 \times (+0.43)\hspace{1cm} } +0.52}
```
...
```math
\mathit{"Film\ \mathbf{nije}\ (\rightarrow$ NEG)\ \mathbf{zanimljiv}" (p=+0.53) \xrightarrow[\hspace{1cm}\times (-1)\hspace{1cm}]{+0.53 } -0.53}
```
...
```math 
\mathit{"Odličan film\mathbf{!}"(\leftarrow\times$ 1.06)} \xrightarrow[\hspace{1cm}\times 1.06\hspace{1cm}]{p=+0.57}+0.60
```
...
```math 
\mathit{"Tako\ \mathbf{dooooosadan} (p=-0.24)..." \xrightarrow[1.27]{\hspace{1cm}\times 1.05^{chr(o)\hspace{1cm}} } "Tako\ \mathbf{dosadan}..."(p=-0.30)}
```

### Emoticons and Emojis
Sentiment of emojis intensities have been utilized by the [Emoji Sentiment Ranking v1.0](https://kt.ijs.si/data/Emoji_sentiment_ranking/index.html) lexicon.
